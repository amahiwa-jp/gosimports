...
func init() {
	...
	flag.BoolVar(&options.FormatOnly, "format-only", false, "if true, don't fix imports and only format. In this mode, gosimports is effectively gofmt, with the addition that imports are grouped into sections.")
}
...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: gosimports [flags] [path ...]\n")
	flag.PrintDefaults()
	os.Exit(2)
}
...
// argumentType is which mode gosimports was invoked as.
type argumentType int

const (
	// fromStdin means the user is piping their source into gosimports.
	fromStdin argumentType = iota

	// singleArg is the common case from editors, when gosimports is run on
	// a single file.
	singleArg

	// multipleArg is when the user ran "gosimports file1.go file2.go"
	// or ran gosimports on a directory tree.
	multipleArg
)
...
func processFile(filename string, in io.Reader, out io.Writer, argType argumentType) error {
	...
	if *srcdir != "" {
		...
		if isFile(*srcdir) {
			...
			// The gosimports on-save hook writes the buffer to a temp file
			// first and runs gosimports before the actual save to newfile.go.
			// The editor's buffer is named "newfile.go" so that is passed to gosimports as:
			//      gosimports -srcdir=/gopath/src/pkg/newfile.go /tmp/gofmtXXXXXXXX.go
...
func gofmtMain() {
	...
	// used to allow gosimports to compile under gccgo, which does not support
